...
$result = mysqli_query($connect, "SELECT *, date_format(birthday,'%d.%m.%Y') as birthday2, date_format(DateVidv,'%d.%m.%Y') as DateVidv2, date_format(DateReg,'%d.%m.%Y') as DateReg2, date_format(DateRegRefusal,'%d.%m.%Y') as DateRegRefusal2 FROM kandidat WHERE $whereSQL ORDER BY checkDateVidv ASC LIMIT $offset,$rows");
